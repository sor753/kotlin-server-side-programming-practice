...
コードスタイルの自動整形に [ktlint](https://pinterest.github.io/ktlint/)、静的解析に [detekt](https://detekt.dev/) を導入しています。
...
> どちらも公式のGradleプラグインでは、このプロジェクトのKotlinバージョン(2.3.21)に起因する非互換エラー（`NoSuchFieldError: HEADER_KEYWORD` / `detekt was compiled with Kotlin 2.0.10 but is currently running with 2.3.21`）が解消できなかったため、Dev Container（`.devcontainer/Dockerfile`）に導入した公式配布の単体実行可能jar（`ktlint`/`detekt`コマンド）を直接呼び出す方式にしています。Gradleプラグインを使わないため、これらのタスクは **Dev Container内でのみ**実行できます。

#### 保存時の自動フォーマット

VS Code拡張機能 `emeraldwalk.RunOnSave`（devcontainer.jsonに導入済み）により、`.kt` ファイル保存時に自動で `ktlint -F` が実行されます（`.vscode/settings.json` の `emeraldwalk.runonsave` で設定）。
